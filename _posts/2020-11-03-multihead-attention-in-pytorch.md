...
这里为简单起见没有实现mask，若要实现，只需要把`k`或者`q`逐点乘以`mask`（假设我们有一个`mask`，需要保留的地方是`1`，需要mask掉的地方是`-inf`， **为数值稳定性考虑，一定把`k`或者`q`加一个很小的数，否则若出现`-inf * 0.0`会产生`NaN`** ），这样两个`Tensor`进行矩阵乘法后，在需要mask掉的位置的分数就是负无穷，Softmax后的注意力分布就是0。
